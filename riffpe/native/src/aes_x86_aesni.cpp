...
namespace riffpe
{
    namespace crypto
    {
        ...
        void AESEngine::set_key(const uint8_t *key, size_t size)
        {
            switch(size)
            {
                case 16:
        ...
        void AESEngine::encrypt_ecb(const uint8_t* in, uint8_t* out, size_t block_count)
        {
            ...
            for(int i=0; i<_round_count+1; ++i)
                key[i] = loadu_mm128i(_key[i].data());

            for(int i=0; i<block_count; ++i)
            {
                aes_state = loadu_mm128i(in);
                in += 16;
                do_aes_enc_transform(aes_state, key, _round_count);
                storeu_mm128i(out, aes_state);
                out += 16;
            }
        }

        void AESEngine::encrypt_cbc(const uint8_t* in, uint8_t* out, size_t block_count, uint8_t* state)
        {
